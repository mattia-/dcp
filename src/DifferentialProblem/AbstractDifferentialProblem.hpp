...
namespace control_problem
{
	...
	class AbstractDifferentialProblem
	{
		...
		public:
			...
			//! Get reference to the problem's parameters [1] (const version)
			/*! 
			 *  \return a const reference to the solver's parameters
			 */
			const dolfin::Parameters& parameters () const;

			//! Get reference to the problem's parameters [2] (non-const version)
			/*! 
			 *  \return a reference to the solver's parameters
			 */
			 ...
			 *  Input arguments are:
			 *  \param coefficientType used to disambiguate between different member variables to choose which integration
			 *  subdomain to set
