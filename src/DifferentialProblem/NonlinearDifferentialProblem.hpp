...
namespace control_problem
{
	...
	template <class T_ResidualForm, class T_JacobianForm>
		...
		{ 
			dolfin::log (dolfin::DBG, "Building NonlinearDifferentialProblem...");
			
			dolfin::log (dolfin::DBG, "Setting up parameters...");
			parameters_.add (dolfin::NonlinearVariationalSolver::default_parameters ());
			parameters_.add ("residual_form_solution_name", residualFormSolutionName);
			if (jacobianFormSolutionName.empty ())
			{
				parameters_.add ("jacobian_form_solution_name", residualFormSolutionName);
			}
			else
			{
				parameters_.add ("jacobian_form_solution_name", jacobianFormSolutionName);
			}
	...
	template <class T_ResidualForm, class T_JacobianForm>
		                ...
		                const std::string& coefficientName = "default")
		{
			...
			else if (coefficientType == "initial_guess")
			{
				...
				if (boost::dynamic_pointer_cast<dolfin::Function> (coefficientValue) != nullptr)
				{
					solution_ = *(boost::dynamic_pointer_cast<dolfin::Function> (coefficientValue));
				}
				else if (boost::dynamic_pointer_cast<dolfin::Expression> (coefficientValue) != nullptr)
				{
					solution_ = *(boost::dynamic_pointer_cast<dolfin::Expression> (coefficientValue));
				}
				else
				{
	...
	template <class T_ResidualForm, class T_JacobianForm>
		                          ...
		                          const control_problem::SubdomainType& subdomainType)
		{
			...
			{
				dolfin::warning ("Cannot set integration subdomain in non linear differential problem. Form type %s unknown",
				                 coefficientType.c_str ());
			}
	...
	template <class T_ResidualForm, class T_JacobianForm>
		...
		solve () 
		{
			...
			{
				dolfin::solve (residualForm_ == 0, solution_, tmpDirichletBCs, jacobianForm_, parameters_);
			}
			else
			{
				dolfin::solve (residualForm_ == 0, solution_, jacobianForm_, parameters_);
			}
		}
