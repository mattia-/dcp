...
/*      uxDofsNum_ ((2*problemData.nx+1)*(2*problemData.ny+1)), uyDofsNum_ (uxDofsNum_),
      ...
      auxiliary_ (pb_.functionSpace())
    {
      ...
      enPtr.reset (new mainEnergyForms::Form_bdTerm (* pb_.mesh()));
      formsNvalues_.emplace ("bdTerm", std::make_pair (enPtr, double ()));
      formsNvalues_["bdTerm"].first->set_coefficient ("dt", dolfin::reference_to_no_delete_pointer (* coefficients_["dt"]));
      formsDepOnSol_.push_back (formsNvalues_["bdTerm"].first);
      formsDepOnOld_.push_back (formsNvalues_["bdTerm"].first);
      formsDepOnDispl_.push_back (formsNvalues_["bdTerm"].first);
      ...
      formsDepOnSol_.push_back (formsNvalues_["grav"].first);

      enPtr.reset (new mainEnergyForms::Form_enPot (* pb_.mesh()));
      formsNvalues_.emplace ("enPot", std::make_pair (enPtr, double ()));
      formsNvalues_["enPot"].first->set_coefficient ("dt", dolfin::reference_to_no_delete_pointer (* coefficients_["dt"]));
      formsNvalues_["enPot"].first->set_coefficient ("f", dolfin::reference_to_no_delete_pointer (* coefficients_["gravityVector"])); 
      ...
      enPtr.reset (new mainEnergyForms::Form_bdGravu (* pb_.mesh()));
      formsNvalues_.emplace ("bdGravu", std::make_pair (enPtr, double ()));
      formsNvalues_["bdGravu"].first->set_coefficient ("f", dolfin::reference_to_no_delete_pointer (* coefficients_["gravityVector"])); 
      formsDepOnSol_.push_back (formsNvalues_["bdGravu"].first);

      enPtr.reset (new mainEnergyForms::Form_bdGravw (* pb_.mesh()));
      formsNvalues_.emplace ("bdGravw", std::make_pair (enPtr, double ()));
      formsNvalues_["bdGravw"].first->set_coefficient ("dt", dolfin::reference_to_no_delete_pointer (* coefficients_["dt"]));
      formsNvalues_["bdGravw"].first->set_coefficient ("f", dolfin::reference_to_no_delete_pointer (* coefficients_["gravityVector"])); 
      formsDepOnDispl_.push_back (formsNvalues_["bdGravw"].first);
      ...
      enPtr.reset (new mainEnergyForms::Form_divEpsGamma (* pb_.mesh()));
      formsNvaluesOnOld_.emplace ("divEpsGamma", std::make_pair (enPtr, double ()));
      formsNvaluesOnOld_["divEpsGamma"].first->set_coefficient ("dt", dolfin::reference_to_no_delete_pointer (* coefficients_["dt"]));
      formsNvaluesOnOld_["divEpsGamma"].first->set_coefficient ("gamma", dolfin::reference_to_no_delete_pointer (* coefficients_["gamma"])); 
      formsDepOnDispl_.push_back (formsNvaluesOnOld_["divEpsGamma"].first);

      enPtr.reset (new mainEnergyForms::Form_divEpsGammaErr (* pb_.mesh()));
      formsNvaluesOnOld_.emplace ("divEpsGammaErr", std::make_pair (enPtr, double ()));
      formsNvaluesOnOld_["divEpsGammaErr"].first->set_coefficient ("dt", dolfin::reference_to_no_delete_pointer (* coefficients_["dt"]));
      formsNvaluesOnOld_["divEpsGammaErr"].first->set_coefficient ("gamma", dolfin::reference_to_no_delete_pointer (* coefficients_["gamma"])); 
      formsDepOnSol_.push_back (formsNvaluesOnOld_["divEpsGammaErr"].first);
      formsDepOnDispl_.push_back (formsNvaluesOnOld_["divEpsGammaErr"].first);
      ...
      enPtr.reset (new mainEnergyForms::Form_inflow (* pb_.mesh()));
      formsNvalues_.emplace ("inflow", std::make_pair (enPtr, double ()));
      formsDepOnSol_.push_back (formsNvalues_["inflow"].first);

      enPtr.reset (new mainEnergyForms::Form_supg (* pb_.mesh()));
      ...
      std::ofstream outputFile (outputFileName_, std::ofstream::out);
      outputFile << "kinEn, viscPow, discr, discr_div, bdTerm, grav, enPot, nbc, sigmaLength, epsgOld, epsg, bdGravu, bdGravwOld, bdGravwNew, divEpsGammaNN, divEpsGammaNNp1, divEpsGammaErrNN, tgDivSol, inflow, supg";
      outputFile << std::endl;
      outputFile.close();
...
//        std::cerr << "processing " << formNvalue.first << std::endl;
      ...
      outputFile.setf ( std::ios::scientific, std::ios::floatfield);
      outputFile.precision (15);
      outputFile << formsNvalues_["kinEn"].second << ',';
      outputFile << formsNvalues_["viscPow"].second << ',';
      outputFile << formsNvalues_["discr"].second << ',';
      outputFile << formsNvalues_["discrDiv"].second << ',';
      outputFile << formsNvalues_["bdTerm"].second << ',';
      outputFile << formsNvalues_["grav"].second << ',';
      outputFile << formsNvalues_["enPot"].second << ',';
      outputFile << formsNvalues_["nbc"].second << ',';
      outputFile << formsNvalues_["sigmaLength"].second << ',';
